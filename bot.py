...
def start(message):
    ...
    if message.text == '/simple':
        ...
        bot.send_message(
            message.chat.id, ("/simple - простая проверка одного квадрата\n"
        ...
        bot.send_message(
            message.chat.id, "Для начала нужно создать игру /create_game")
...
def get_round_id(message):
    ...
    try:
        ...
        if 1 <= round_id and round_id <= max_rounds:
            ...
            bot.send_message(
                message.chat.id, "Некорректный ввод, попробуй ещё раз")
    except Exception as exc:
        print(exc)
        bot.send_message(
            message.chat.id, "Что-то пошло не так, попробуй ещё раз")
...
def round_number_processor(message):
    ...
    if round_id in round_set:
        bot.send_message(
            message.chat.id, "Раунд уже зарегистрирован. Хочешь изменить набор чисел?\n/yes\t /no")
    else:
        bot.send_message(
...
def get_round_numbers(message):
    ...
    try:
        if message.text == '/yes':
            ...
            bot.reply_to(
                message, "Отлично, набор чисел сохранён, раунд изменён на {}".format(round_id))
    except Exception as exc:
        print(exc)
        bot.send_message(
            message.chat.id, "Что-то пошло не так, попробуй ещё раз")
...
def get_user_id(message):
    ...
    try:
        ...
        if 1 <= user_id and user_id <= max_users:
            bot.send_message(
                message.chat.id, "Отлично. Теперь введи 25 чисел от 1 до 13 - значения квадрата")
            bot.register_next_step_handler(message, get_square)
            sessions[message.chat.id].user_id = user_id
        else:
            bot.send_message(
                message.chat.id, "Некорректный ввод, попробуйте ещё раз")
    except Exception as exc:
        print(exc)
        bot.send_message(
            message.chat.id, "Что-то пошло не так, попробуй ещё раз")
...
def get_square(message):
    ...
    try:
        ...
        if not valid_set(numbers):
            ...
            bot.send_message(
                message.chat.id, "Набор чисел не совпадает с набором в раунде. \nДолжен быть: {}".format("".join(map(str, sorted(set)))))
        else:
            args = ['./quad']
            for n in numbers:
                args.append(str(n))
            ...
            bot.send_message(message.chat.id, "Счёт: {}\nПользоваель: {}\nРаунд: {}\nСохранить?\n /yes \t /no".format(
                sessions[message.chat.id].score, sessions[message.chat.id].user_id, sessions[message.chat.id].round_id))
...
def save_score(message):
    try:
        ...
        if message.text == "/yes":
            ...
            service.spreadsheets().values().update(spreadsheetId=spreadsheetId, valueInputOption="USER_ENTERED",
                                                   range="Рабочая!{0}{1}:{0}{1}".format(get_column_letter(
                                                       round_id + offset_cols), user_id + offset_rows),
...
def get_image_from_text(text):
    ...
    ImageDraw.Draw(img).multiline_text(
        (20, 10), text, font=font, fill=(0, 0, 0))
    return img
...
def create_sheet():
    spreadsheet = service.spreadsheets().create(body={
        ...
        'sheets': [{'properties': {'sheetType': 'GRID',
                                   'sheetId': 0,
                                   'title': 'Рабочая',
                                   'gridProperties': {'rowCount': max_users + 2, 'columnCount': max_rounds + 10}}},
    ...
    print('Ссылка на таблицу результатов: https://docs.google.com/spreadsheets/d/{}'.format(spreadsheetId))
    service.spreadsheets().values().update(spreadsheetId=spreadsheetId, valueInputOption="USER_ENTERED",
    ...
    service.spreadsheets().values().update(spreadsheetId=spreadsheetId, valueInputOption="USER_ENTERED",
                                           ...
                                           body={"majorDimension": "COLUMNS",
                                                 "values": [["=SUM({0}{1}:{1})".format(get_column_letter(offset_cols), offset_rows + i) for i in range(1, max_users + 1)]]}
                                           ).execute()

    service.spreadsheets().values().update(spreadsheetId=spreadsheetId, valueInputOption="USER_ENTERED",
                                           range="Рабочая!{0}{1}:{2}{1}".format(
                                               get_column_letter(offset_cols), 1, get_column_letter(offset_cols + max_rounds - 1)),
                                           body={"majorDimension": "ROWS",
    ...
    service.spreadsheets().values().update(spreadsheetId=spreadsheetId, valueInputOption="USER_ENTERED",
                                           range="Отсортированная!A{0}:A{0}".format(
                                               offset_rows + 1),
                                           body={"majorDimension": "ROWS",
                                                 "values": [["=SORT(Рабочая!A{0}:{1}{2};3;FALSE)".format(offset_rows + 1, get_column_letter(offset_cols + max_rounds - 1), offset_rows + max_users)]]}
                                           ).execute()
...
def send_table(message):
    bot.send_message(
        message.chat.id, 'Ссылка на таблицу результатов: https://docs.google.com/spreadsheets/d/{}'.format(spreadsheetId))
